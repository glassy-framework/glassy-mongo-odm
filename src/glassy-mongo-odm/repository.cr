...
module Glassy::MongoODM
  abstract class Repository(T)
    ...
    abstract def to_bson(entity : T) : BSON
    abstract def from_bson(bson : BSON) : T
    abstract def collection_name : String
    abstract def database_name : String
    abstract def find_by(query : Hash)
    abstract def get_primary_key(entity : T) : BSON::ObjectId?
    abstract def set_primary_key(entity : T, id : BSON::ObjectId)
    ...
    def find_by_id(id : String) : T?
      find_by_id(BSON::ObjectId.new(id))
    end

    def find_by_id(id : BSON::ObjectId) : T?
      find_one_by({"_id" => id})
    end
    ...
    macro generate_methods
      ...
      {% else %}
        ...
        def get_primary_key(entity : T) : BSON::ObjectId?
          get_primary_key_macro
        end

        def set_primary_key(entity : T, id : BSON::ObjectId)
